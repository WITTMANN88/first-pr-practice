...
## Постоянный бот (роли и дальше — тикеты, XP, музыка...)

В отличие от `provision.js` (разовый запуск), `bot.js` должен работать
постоянно, пока сервер живёт — иначе меню ролей и остальные фичи перестают
отвечать.
...
При старте бот сам публикует меню самостоятельного выбора ролей в
`#choose-your-roles` (если его там ещё нет) — два выпадающих списка: игры и
дополнительные роли (Politics / Other Games). Участник выбирает нужное, бот
сам выдаёт и снимает роли, ничего вручную делать не нужно.

Не закрывай окно консоли, пока хочешь, чтобы бот отвечал — окно закрыл, бот
оффлайн. Постоянный хостинг (чтобы не держать свой ПК включённым 24/7) —
отдельный вопрос, разберём отдельно, когда дойдём до этого.

Следующие фичи (тикеты, музыка, XP, антирейд, YouTube/Twitch-фид, клип
недели) будут добавляться отдельными модулями в `features/` и подключаться
в `bot.js`.
