...
async function main() {
  ...
  const politicsRole = await findOrCreateRole(guild, 'Politics', { color: 'DarkRed', mentionable: false });
  const otherGamesRole = await findOrCreateRole(guild, 'Other Games', { color: 'Blue', mentionable: false });
  const gameRoles = {};
