...
> **Статус.** Код собирается и проходит 319 unit-тестов в CI на `windows-latest`,
...
Температура процессора обновляется каждые 3 с: бар (красный от 85 °C) и
...
выбирает пользователь). ISLC и MakuTweaker — прямой загрузкой по HTTPS с
прогресс-баром в папку «Загрузки» и запуском установщика. Ссылки закреплены
на конкретных версиях в `src/STAKEOUT/Services/SoftwareInstallService.cs`; при
выходе новых версий обновите там URL и имя файла.
...
dotnet test  STAKEOUT.sln -c Release     # 319 тестов, работают на любой ОС
...
flowchart TB
    ...
    T["<b>STAKEOUT.Tests</b><br/>xUnit · 319 тестов"]
...
- Загрузки — только HTTPS с проверкой отзыва сертификатов.
...
319 тестов xUnit (`src/STAKEOUT.Tests`, net8.0, любая ОС): шифрование лога,
...
резолвер путей, спарклайн, дизайн-данные и XAML-lint (каждый `{Binding}`
ссылается на существующее свойство).
...
flowchart LR
    ...
    TEST["Build & test<br/>windows-latest<br/>0 warnings, 319 тестов"] --> PUB
...
- Установщики ISLC и MakuTweaker проверяются только транспортом (HTTPS +
  отзыв сертификатов), без закреплённого хеша файла.
